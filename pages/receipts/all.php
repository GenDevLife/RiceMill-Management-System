<?php
/**
 * Receipt - Service + Product
 * หน้าใบเสร็จ (บริการ + สินค้า)
 */

$basePath = '../../';
$additionalCss = ['receipt.css'];
...
$productPrice = isset($_POST['product_price']) ? (float)$_POST['product_price'] : 0;
...
try {
    // บันทึกบริการ
    $db->insertOrderService($memberId, $serviceType, $weightKg, $servicePrice);
    
    // บันทึกสินค้า (ถ้ามี)
    if ($productPrice > 0) {
        $db->insertOrderProduct($memberId, $riceBranKg, $huskKg, $riceChunksKg, $brokenRiceKg, $productPrice);
    }
    
    // เพิ่มคะแนน
    $db->addMemberPoints($memberId, $earnedPoints);
    
    // ดึงข้อมูลสมาชิกใหม่ (หลังเพิ่มคะแนน)
    $member = $db->getMember($memberId);
...
<body>
    <div class="receipt-container">
        ...
        <div class="receipt-header">
            <img src="<?php echo $basePath; ?>assets/images/TechTeam.png" 
                 alt="Logo" 
                 class="receipt-logo"
        ...
        <table class="receipt-table">
            ...
            <tbody>
                <tr>
                    <td><?php echo getServiceName($serviceType); ?></td>
                    <td><?php echo number_format($weightKg); ?> กก.</td>
                    <td><?php echo number_format($servicePrice, 2); ?></td>
                </tr>
